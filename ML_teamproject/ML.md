...
`/sniffer/schema.py`. Metadata and label fields are not predictive
...
Dataset identity and training-run identity are separate.

- `dataset_version` identifies a raw pcap and its processed feature dataset.
- `run_version` identifies one training execution and its artifacts.

One dataset can feed multiple runs, and one run can consume one or more datasets.
...
run_v000001/
  randomforest_v000001.joblib
  metrics_run_v000001.json
  detection_run_v000001.executed.ipynb
```
...
Every training run must be reproducible from tracked inputs and configuration.
Each run must record at least:

```text
run_version
dataset_versions
source_processed_csv_files
label mapping
...
runs/run_vNNNNNN/detection_run_vNNNNNN.executed.ipynb
```
...
train           Execute the notebook and generate run bundle artifacts
...
```text
runs/run_vNNNNNN/metrics_run_vNNNNNN.json
metrics/metrics_summary.csv
runs/run_vNNNNNN/confusion_matrix_run_vNNNNNN.csv
```

The summary CSV must append one row per training run and include
run version, dataset versions, selected hyperparameters reference, and all
